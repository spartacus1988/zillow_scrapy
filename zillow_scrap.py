...
def update_csv_cell(ZillowSpider, browser):
	...
	with open(filepath, 'r+') as csvfile:
		...
		for current_line in reader:
			...
			if current_line[10] == 'url':
				...
				# current_line.append('priceSqft')
				# lines.append(current_line)
			else:
				zestimate , zRange, builtIn, builtBy, comName, parking, school, parking_details, lastSold, priceSqft = ZillowSpider.get_one_request(browser, current_line[10])
	...
	columns = ["address", "city", "state", "zip", "price", "sqft", "bedrooms",
           "bathrooms", "days_on_zillow", "sale_type", "url", "zestimate", "zRange", "builtIn", "builtBy", "comName", "parking", "school", "parking_details", "lastSold", "priceSqft"]
